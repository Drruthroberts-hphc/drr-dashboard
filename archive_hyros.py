#!/usr/bin/env python3
"""Archive the Hyros Lead conversion action (legacy, no longer receiving data)."""
import os
...
try:
    ...
    print(f'✅ Archived: {response.results[0].resource_name}')
...
for row in ga_service.search(customer_id=customer_id, query='''
    ...
    WHERE conversion_action.id = 1037552323
'''):
