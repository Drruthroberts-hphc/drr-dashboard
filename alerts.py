...
from config import ALERT_THRESHOLDS, ALERT_EMAIL, GMAIL_CREDENTIALS, GMAIL_TOKEN
...
def send_alert_email(alerts, week_ending_date):
    ...
    msg['To'] = ALERT_EMAIL
    msg['Subject'] = f"DRR Dashboard Alert - {len(alerts)} threshold(s) breached - Week {week_ending_date}"
...
def _format_weekly_summary(all_data, alerts, week_ending_date):
    ...
    html += """
            <p style="margin-top: 25px; color: #999; font-size: 11px; text-align: center;">
...
def send_weekly_summary(all_data, alerts, week_ending_date):
    ...
    msg = MIMEMultipart('alternative')
    msg['To'] = ALERT_EMAIL
    msg['Subject'] = f"DRR Weekly Dashboard Summary - Week ending {week_ending_date}"
    ...
    try:
        ...
        logger.info(f"Weekly summary sent to {ALERT_EMAIL}")
        return True
    except Exception as e:
